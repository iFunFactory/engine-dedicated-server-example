...
void AShooterGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
  ...
  fun::FunapiDedicatedServer::SendReady(
    [this](fun::FunapiDedicatedServer::HttpResponse http_response)
  {
    if (http_response.code != 0)
    {
      // 호스트매니저로 SendReady를 보내는 데 실패했으므로 더 이상 진행할 수 없습니다.
      // SendReady를 받지 못한 호스트매니저는 프로세스를 자동으로 종료합니다.
      UE_LOG(LogTemp, Error, TEXT("Failed to send SendReady message to Host Manager. The Process will be killed by HostManager"));
    }
    else
    {
      UE_LOG(LogTemp, Log, TEXT("Succeed to send SendReady message to Host Manager"));
    }
  });
}
...
void AShooterGameMode::FinishMatch()
{
  ...
  if (IsMatchInProgress())
  {
    ...
    fun::FunapiDedicatedServer::SendResult(ouput_fstring,
      [this](fun::FunapiDedicatedServer::HttpResponse http_response)
      {
        if (http_response.code != 0) {
          // 호스트매니저로 SendResult를 보내는 데 실패했으므로 더 이상 진행할 수 없습니다.
          UE_LOG(LogTemp, Error, TEXT("Failed to send SendResult message to Host Manager."));

          // 메세지 전송부분은 플러그인 내부 소스로 인터넷 환경이 불안정 하지 않다면
          // HostManager 에게 메세지 전송이 실패하는 상황은 발생해서는 안된다.
          FGenericPlatformMisc::RequestExit(false);
          return;
        }

        UE_LOG(LogTemp, Log, TEXT("Succeed to send SendResult message to Host Manager."));
...
void AShooterGameMode::PreLogin(const FString& Options, const FString& Address, const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage)
{
  ...
  if (fun::FunapiDedicatedServer::AuthUser(uid, token, error_message) == false) {
    return;
  }
...
APlayerController* AShooterGameMode::Login(UPlayer* NewPlayer, ENetRole InRemoteRole, const FString& Portal, const FString& Options, const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage)
{
  ...
  // 호스트 매니저에게 새 클라이언트가 접속했다고 알립니다.
  fun::FunapiDedicatedServer::SendJoined(uid,
    [this](fun::FunapiDedicatedServer::HttpResponse http_response)
    {
      if (http_response.code != 0) {
        UE_LOG(LogTemp, Error, TEXT("Failed to send SendJoined message to HostManager."));
        ...
        UE_LOG(LogTemp, Log, TEXT("Succeed to send SendJoined message to HostManager."));
      }
    }
  );
...
void AShooterGameMode::Logout(AController* Exiting)
{
  ...
  fun::FunapiDedicatedServer::SendLeft(uid,
    ...
    {
      if (http_response.code != 0) {
        UE_LOG(LogTemp, Error, TEXT("Failed to send SendLeft message to HostManager."));
        ...
        UE_LOG(LogTemp, Log, TEXT("Succeed to send SendLeft message to HostManager."));
      }
    }
  );
}

...
{
  fun::FunapiDedicatedServer::StartedHandler func =
    [this]()
    {
#if PLATFORM_PS4 == 0
  ...
  if (IsDedicatedServerInstance()) {
    // FunapiDedicatedServer 의 초기화를 진행한다.
    // 초기화 과정은 다음과 같다.
    // 1. FunapiDedicatedServer 플러그인 옵션 설정, 사용자 옵션 문자열 저장
    // 2. HostManager 와 버전정보가 일치하는지 확인.
    // 3. HostManager 에서 게임진행에 관련된 정보들을 받아 저장한다.
    //
    // FunapiDedicatedServer::SetUserDataCallback, FunapiDedicatedServer::match_data_handler_
    // 위의 handler를 등록하여 저장이 되었을때 확인이 가능하다.
    fun::FunapiDedicatedServer::SetStartedCallback(func);
    fun::FunapiDedicatedServer::SetDisconnectedCallback(
      [this]() {
        UE_LOG(LogTemp, Error, TEXT("Disconnected from host manager. Please check the host manager"));
        // 호스트 매니저와 접속이 끊겼을 때 처리를 추가합니다.
        // ex) 게임 데이터를 파일로 저장.
        }
    );

    fun::FunapiDedicatedServer::Start(FString("1.2.3.2500"));
...
bool UShooterGameInstance::TestFunapiServerConnect()
{
  ...
  // 실행시 FunapiServer, FunapiServerPort 인자값을 넣어주지 않았다면
  // 아래의 서버에 접속에 관련된 정보를 수정해야 합니다.
  FString funapi_server_ip = "10.10.2.250";
  FString funapi_server_port = "8012";
  fun::FunEncoding test_encoding = fun::FunEncoding::kJson;
  // fun::FunEncoding test_encoding = fun::FunEncoding::kProtobuf;

  if (FParse::Value(FCommandLine::Get(), *funapi_server_field, funapi_server_ip))
  {
    funapi_server_ip = funapi_server_ip.Replace(TEXT("="), TEXT("")).Replace(TEXT("\""), TEXT("")); // replace quotes
    UE_LOG(LogTemp, Log, TEXT("%s = '%s'"), *funapi_server_field, *funapi_server_ip);
  }

  if (FParse::Value(FCommandLine::Get(), *funapi_server_port_field, funapi_server_port))
  {
    funapi_server_port = funapi_server_port.Replace(TEXT("="), TEXT("")).Replace(TEXT("\""), TEXT("")); // replace quotes
    UE_LOG(LogTemp, Log, TEXT("%s = '%s'"), *funapi_server_port_field, *funapi_server_port);
  }

  if (funapi_server_ip.IsEmpty() || funapi_server_port.IsEmpty())
  {
    UE_LOG(LogTemp, Error, TEXT("Dedicated Sample must set funapi_server_ip, funapi_server_port"));
    return false;
  }



  std::string kServer = TCHAR_TO_UTF8(*funapi_server_ip);
  int kPort = FCString::Atoi(*funapi_server_port);
  ...
  session_->AddSessionEventCallback([this, test_encoding](const std::shared_ptr<fun::FunapiSession> &session,
    ...
    if (type == fun::SessionEventType::kOpened) {
      UE_LOG(LogTemp, Display, TEXT("Session initiated: %s"), *FString(session_id.c_str()));
      FGuid fguid;
      playerId = fguid.NewGuid().ToString(EGuidFormats::Digits);

      if (test_encoding == fun::FunEncoding::kJson) {
        // 데디케이티드 서버 로그인 요청 예제
        // 클라이언트는 다음 메세지 형태로 로그인 요청을 해야함.
        // "account_id" = "id"
        // "platform" = "guset"
        // "access_token" = "your access_token(google, facebook)"
        TSharedRef<FJsonObject> json_object = MakeShareable(new FJsonObject);

        json_object->SetStringField(FString("account_id"), playerId);
        json_object->SetStringField(FString("platform"), FString("window"));
        json_object->SetStringField(FString("access_token"), FString("temp_access_token"));
      ...
      // 샘플 프로젝트에서는 Protobuf 인코딩은 사용하지 않는다.
  ...
  session_->AddJsonRecvCallback([this](const std::shared_ptr<fun::FunapiSession> &session,
    ...
    if (msg_type.compare("_sc_dedicated_server") == 0)
    {
      ...
      if (json_object->HasField(FString("redirect")))
      {
        ...
        // 서버에서는 다음과 같은 redriect 메세지를 전달 받습니다.
        // "host"  : "ip"
        // "port"  :  0
        ...
        // 리다이렉션 필드가 존재하지 않는다면 샘플 프로젝트상 버그로 프로그램을 종료합니다.
        UE_LOG(LogTemp, Error, TEXT("Invalid 'dedicated redirect' message"));
        FGenericPlatformMisc::RequestExit(false);
      }
    }
    else if (msg_type.compare("login") == 0) {
      // 데디케이티드 서버 스폰 요청 예제
      // 클라이언트는 다음 메세지 형태로 데디케티드 서버 스폰 요청해야 합니다.
      //{
      //  "account_id" : "id",
      //  "user_data" : {
      //    "level": 60,
      //    "mmr_score" : 1000,
      //    "my_match_key1" : "my_match_value1",
      //    "my_match_key2" : "my_match_value2",
      //    "my_match_key3" : "my_match_value3"
      //  },
      //  "match_type": 1
      //}

      // match_type 에 대한 정보는 다음과 같습니다.
      // 1인 생성(매치메이킹 패스): 0
      // 2v2: 1
      // 3v3: 3
      // 6v6: 6
      ...
      {
        TSharedPtr< FJsonObject > user_data_json_object = MakeShareable(new FJsonObject);
        user_data_json_object->SetNumberField(FString("level"), 60);
        user_data_json_object->SetNumberField(FString("mmr_score"), 1000);
        user_data_json_object->SetStringField(FString("my_match_key1"), FString("my_match_value1"));
        user_data_json_object->SetStringField(FString("my_match_key2"), FString("my_match_value2"));
        user_data_json_object->SetStringField(FString("my_match_key3"), FString("my_match_value3"));

        json_object->SetObjectField(FString("user_data"), user_data_json_object);
      }

      json_object->SetNumberField(FString("match_type"), 0);
      ...
      session_->SendMessage("match", json_stiring);
      UE_LOG(LogTemp, Display, TEXT("Send dedicated server match message"));

    }
    else
    {
      UE_LOG(LogTemp, Warning, TEXT("unavailable msg_type: %s"), *FString(msg_type.c_str()));
    }
  });
  ...
  session_->AddProtobufRecvCallback([this](const std::shared_ptr<fun::FunapiSession> &session,
    ...
    // NOTE(sungjin) 현재 샘플 프로젝트에서 Protobuf는 사용하지 않습니다.
  });
